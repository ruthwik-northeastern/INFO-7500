...
describe("NFTDutchAuction_ERC20", () => {
    ...
    beforeEach(async () => {
        ...
        const NFTcontractFactory = new ContractFactory(
            NFTDutchAuction_ERC20Bids.abi,
            NFTDutchAuction_ERC20Bids.bytecode,
            seller
        );
        contract = await NFTcontractFactory.deploy(ktoken_contract.address,
            mint_contract.address, 0, 100, 5, 10);
    ...
    describe("Placing bid", () => {
        it("Not allowed to bid if the user doesn't have the the required allowance", async () => {
            ...
            await expect(contract.functions.bid(600)).to.be.rejectedWith("ERC20: insufficient allowance");
        });
        it("Reject a bid if bid price is less than initial price ", async () => {
            ...
            await expect(contract.functions.bid(100)).to.be.rejectedWith("Bid value is less than current price.");
        ...
        it("No more bidding after the auction  ", async () => {
        ...
        it("should reject a bid if auction is closed", async () => {
            ...
            await expect(contract.functions.bid(600)).to.be.rejectedWith("Winner has been chosen. No more bidding. Auction closed");
        });
